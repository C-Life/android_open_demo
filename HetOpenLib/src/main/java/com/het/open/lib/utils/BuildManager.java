...
public class BuildManager {
    ...
    public static void setStatusTrans(Activity activity, int parentType, View view, View rootView,int defColor) {
        if (isSDKLater19()) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);//透明状态栏
            if (rootView != null) {
                if (defColor != 0) {
                    rootView.setBackgroundColor(defColor);
                } else {
                    rootView.setBackgroundColor(activity.getBaseContext().getResources().getColor(R.color.translate));
                }
            }
            //设置沉浸模式时,页面会填充状态栏位置,设置合适的margin值使页面从状态栏下开始布局
            if (parentType == 1) {
    ...
    public static void setStatusTrans(Activity activity) {
        if (isSDKLater19()) {
            ...
            statusBarView.setBackgroundColor(activity.getBaseContext().getResources().getColor(R.color.translate));
