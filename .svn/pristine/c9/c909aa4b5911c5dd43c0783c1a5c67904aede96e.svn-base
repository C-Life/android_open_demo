 ...
 * 设备小类列表适配器
 */
public class AdapterDeviceSubtypeList extends HelperRecyclerViewAdapter<DeviceSubModel> {


    public AdapterDeviceSubtypeList(Context context, int... layoutIds) {
        super(context, layoutIds);
    }

    @Override
    protected void HelperBindData(HelperRecyclerViewHolder holder, int i, DeviceSubModel deviceSubModel) {
        ...
        String imageUrl = deviceSubModel.getProductIcon();
        ...
        String name = deviceSubModel.getDeviceSubtypeName();
        if (!TextUtils.isEmpty(name)) {
            tvwName.setText(name);
        } else {
            tvwName.setText("");
        }
        tvwMac.setVisibility(View.VISIBLE);
        String productCode = deviceSubModel.getProductCode();
        if (!TextUtils.isEmpty(productCode)) {
            tvwMac.setText(productCode);
            tvwMac.invalidate();
        }
    }
