...
public class DeviceH5Persenter extends BaseHetPresenter<BaseHetView> {
     ...
     * 真实设备h5页面的url
     *
     * @param type      1代表快捷键   2代表index页面
     * @param context   上下文
     * @param model      设备对象
     */
    public void loadUrlFromDeviceBean(int type, Context context, DeviceModel model, HtmlFiveManager htmlFiveManager) {
        unRegisterUrl(model);
        RxManage.getInstance().register(HetH5PlugEvent.HET_EVENT_H5_PLUG_GET_LOCAL_URL_SUCCESS + model.getProductId(), o -> {
            if (htmlFiveManager != null) {
                ...
                path += type == 1 ? "/appBtnList.html" : "/index.html";
